<?php
  require "./vendor/jsmin.php";
  require "./vendor/cssmin.php";
  App::uses('AppHelper', 'View/Helper'); 
 
  class CombinatorHelper extends AppHelper {
    var $libs = array('js' => array(), 'css' => array()),
        $viewLibs = array('js' => array(), 'css' => array()),
        $inline_code = array('js' => array(), 'css' => array()),
        $basePath = null,
        $cachePath = null,
        $extraPath = null;
    ...
    function inline_scripts($type) {
      switch($type) {
        case 'js':
          return $this->get_inline_js($this->libs[$type]);
        case 'css':
          return $this->get_inline_css($this->libs[$type]);
        default:
          $this->libs['js'] = array_merge($this->libs['js'], $this->viewLibs['js']);
          $outputJs = $$this->get_inline_js($this->libs['js']);
          $outputCss = $this->get_inline_css($this->libs['css']);
          return $outputCss . "\n" . $outputJs;
      }
    }
    ...
    private function generate_filename($type) {
      ...
      $hash = md5(serialize($this->libs[$type]) . '_' . serialize($this->inline_code[$type]));
    ...
    private function get_js_html($cachefile, $async) {
      if (false === file_exists($this->cachePath['js'] . '/' . $cachefile)) {
        ...
        foreach($this->libs['js'] as $lib) {
          $filePath = $this->basePath['js'] . '/' . $lib;
          if (false === file_exists($filePath)) {
            $file_content .= file_get_contents($lib);
          } else {
    ...
    private function get_css_html($cachefile) {
      if (false === file_exists($this->cachePath['css'] . '/' . $cachefile)) {
        ...
        foreach($this->libs['css'] as $lib) {
          $filePath = $this->basePath['css'] . '/' . $lib;
          if (false === file_exists($filePath)) {
            $file_content .= file_get_contents($lib);
          } else {
            $file_content .= file_get_contents($filePath);
    ...
    private function get_inline_css($files) {
      $file_content = '';
      foreach($files as $file) {
        $filePath = $this->basePath['css'] . '/' . $file . ".css";
        if (false === file_exists($filePath)) {
      ...
      return '<style type="text/css">' . $file_content . '</style>';
    }

    private function get_inline_js($files) {
      $file_content = '';
      foreach($files as $file) {
        $filePath = $this->basePath['js'] . '/' . $file . ".js";
        if (false === file_exists($filePath)) {
          $file_content .= file_get_contents($file);
        } else {
          $file_content .= file_get_contents($filePath);
      ...
      return '<script type="text/javascript">' . $file_content .'</script>';
    }
